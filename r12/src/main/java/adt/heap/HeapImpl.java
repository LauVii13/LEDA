...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
  ...
  private void heapify(int position) {
    ...
    if (leftIndex < size() && comparator.compare(heap[leftIndex], heap[largest]) > 0) {
      largest = leftIndex;
    }

    if (rightIndex < size() && comparator.compare(heap[rightIndex], heap[largest]) > 0) {
      largest = rightIndex;
    }
  ...
  public void insert(T element) {
    ...
    if (element != null) {
      ...
      int insertPosition = this.index;

      while (insertPosition > 0 && comparator.compare(element, this.heap[parent(insertPosition)]) > 0) {
        this.heap[insertPosition] = this.heap[parent(insertPosition)];
        insertPosition = parent(insertPosition);
      }

      this.heap[insertPosition] = element;
    }
  }
